...
VOID DS4_TimerResponse( VOID )
{
  ...
  DS4_Anim.GlobalTime = (FLT)(t - StartTime) / CLOCKS_PER_SEC;
  if (!DS4_Anim.IsPause)
  {
    DS4_Anim.Time = (FLT)(t - PauseTime - StartTime) / CLOCKS_PER_SEC;
    DS4_Anim.DeltaTime = (FLT)(t - OldTime) / CLOCKS_PER_SEC;
  }
  else
  {
    PauseTime += t - OldTime;
  ...
  {
    DS4_Anim.FPS = FrameCount / ((FLT)(t - OldFPSTime) / CLOCKS_PER_SEC);
    OldFPSTime = t;
    FrameCount = 0;
  }
  OldTime = t;
